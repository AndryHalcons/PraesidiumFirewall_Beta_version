...
switch ($chain) {
    ...
    case 'url_port_profile':     get_url_port_profile_delete($chain); break;
    case 'url_list':  get_url_list_delete(); break;
    case 'url_network_list':  get_url_network_list_delete(); break;
    case 'url_listen_ports':  get_url_listen_ports_delete(); break;
    case 'url_networks_list_profile':  get_url_networks_list_profile_delete($chain); break;
    default:
...
function get_url_networks_list_profile_delete($chain) {
    require __DIR__ . '/validation_url_filter.php';
    $path = '/var/www/config/squid_config/squid_policies.json';
    $raw = file_get_contents($path);
    if ($raw === false) {
    ...
    foreach ($json['squid']['url_networks_list_profile'] as $i => $entry) {
        if (($entry['rule']['id'] ?? '') === $id) {
            unset($json['squid']['url_networks_list_profile'][$i]);
            $json['squid']['url_networks_list_profile'] = array_values($json['squid']['url_networks_list_profile']);
            $found = true;
            break;
        }
    }
...
function get_url_port_profile_delete($chain) {
    require __DIR__ . '/validation_url_filter.php';
    $path = '/var/www/config/squid_config/squid_policies.json';
    $raw = file_get_contents($path);
    if ($raw === false) {
...
function get_url_list_delete() {
    ...
    $filePath = '/var/www/config/squid_config/squid_folder/conf.d/domain_list/' . basename($filename);
...
function get_url_network_list_delete() {
    require __DIR__ . '/validation_url_filter.php';
    $input = json_decode(file_get_contents('php://input'), true);
    $filename = $input['file'] ?? '';
    ...
    validate_url_network_list($filename);
    // Construir la ruta completa al archivo
    // Build the full path to the file
    $filePath = '/var/www/config/squid_config/squid_folder/conf.d/ip_list/' . basename($filename);
