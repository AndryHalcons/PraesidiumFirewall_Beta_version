function renderTableFromPuser(P_user) {
  const endpoint = "/users/users_table/get_table_structure.php";
  const param = `table=${P_user}`;

  fetch(`${endpoint}?${param}`)
    .then(response => response.json())
    .then(data => {
      const container = document.getElementById(`${P_user}_table`);
      ...
      addBtn.textContent = LANG["add_user"] || "Add user";
      addBtn.onclick = () => add_Puser_policy(P_user, columns);
      ...
      loadTableContentPuser(P_user, columns);
    })
    .catch(error => {
      const container = document.getElementById(`${P_user}_table`);
      if (container) {
...
function loadTableContentPuser(P_user, columns) {
  const endpoint = "/users/users_table/get_table_content.php"; 
  const param = `table=${P_user}`;
  fetch(`${endpoint}?${param}`)
    .then(response => response.json())
    .then(data => {
      const tbody = document.querySelector(`#${P_user}_table table tbody`);
      if (!tbody) return;
      ...
      const formEndpoint = "/users/users_table/get_forms_from_table.php";
      fetch(`${formEndpoint}?table=${P_user}`)
        .then(res => res.json())
        .then(formConfig => {
          rules.forEach(rule => {
            const tr = document.createElement("tr");
            ...
            editBtn.onclick = () => edit_Puser_policy(P_user, rule, columns, tr, editBtn, saveBtn);
            saveBtn.onclick = () => save_Puser_policy(P_user, rule, columns, tr, editBtn, saveBtn);

            const deleteBtn = document.createElement("button");
            deleteBtn.textContent = LANG["delete"] || "Eliminar";
            deleteBtn.onclick = () => delete_Puser_policy(P_user, rule);
...
function edit_Puser_policy(P_user, rule, columns, targetRow, editBtn, saveBtn) {
  const endpoint = "/users/users_table/get_forms_from_table.php";
  const param = `table=${P_user}`;
...
function save_Puser_policy(P_user, rule, columns, targetRow, editBtn, saveBtn) {
  const cells = targetRow.querySelectorAll("td");
  const updatedRule = {};
  ...
  fetch(`${endpoint}?${param}`)
    ...
    .then(formConfig => {
      ...
      sendPuserPolicy(P_user, updatedRule, columns, () => {
        columns.forEach((key, i) => {
          const td = cells[i + 1];
...
function add_Puser_policy(P_user, columns) {
  const endpoint = "/users/users_table/get_forms_from_table.php";
  const param = `table=${P_user}`;
  ...
  fetch(`${endpoint}?${param}`)
    ...
    .then(formConfig => {
      ...
      title.textContent = `Agregar política a ${P_user}`;
...
function sendPuserPolicy(P_user, updatedRule, columns, onSuccess) {
  const payload = {
    table: P_user,
    rule: updatedRule
  };
  ...
  fetch("/users/users_table/get_update_user.php", {
    method: "POST",
    headers: {
...
function delete_Puser_policy(P_user, rule) {
  if (!confirm("¿Estás seguro de que quieres eliminar este usuario?")) {
    return; 
  }

  const endpoint = "/users/users_table/get_delete_user.php";
  ...
  fetch(endpoint, {
    ...
    .then(result => {
      if (result.success) {
        loadTableContentPuser(P_user, Object.keys(rule));
      } else {
