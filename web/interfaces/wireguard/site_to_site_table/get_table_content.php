...
$columns = wireguard_read_structure('wireguard_site_to_site');
...
$rows = [];
foreach (($config['site_to_site'] ?? []) as $name => $entry) {
    $entry['name'] = $name;
...
echo json_encode(['wireguard_site_to_site' => $rows], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
?>
