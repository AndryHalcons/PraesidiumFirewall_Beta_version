...
function save_iface(currentAlias, rule, columns, targetRow, editBtn, saveBtn) {
  const cells = targetRow.querySelectorAll("td");
  const updatedRule = {};
  ...
  fetch(`${endpoint}?${param}`)
    ...
    .then(formConfig => {
      ...
      send_iface(currentAlias, updatedRule, columns, () => {
        columns.forEach((key, i) => {
          const td = cells[i + 1];
        ...
        loadTableContent_iface(currentAlias, columns); // ✅ Solo si todo fue OK
      });
    })
    .catch(error => {
      console.error("Error al cargar configuración de formulario:", error);
...
function add_iface(currentAlias, columns) {
  const endpoint = "/interfaces/interfaces_table/get_forms_from_table.php";
  const param = `table=${currentAlias}`;
...
function send_iface(currentAlias, updatedRule, columns, onSuccess) {
  const payload = {
    table: currentAlias,
    rule: updatedRule
  };
  ...
  fetch("/interfaces/interfaces_table/get_update_interface.php", {
    method: "POST",
    headers: {
...
function delete_iface(currentAlias, rule) {
  ...
  const endpoint = "/interfaces/interfaces_table/get_delete_interface.php";
  ...
  fetch(endpoint, {
    ...
    .then(result => {
      if (result.success) {
        loadTableContent_iface(currentAlias, Object.keys(rule));
      } else {
...
renderTableInterface(currentAlias)