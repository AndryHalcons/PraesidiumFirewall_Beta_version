...
$originalCount = count($rulesJson['bpfilter']);

$rulesJson['bpfilter'] = array_values(array_filter($rulesJson['bpfilter'], function ($entry) use ($idToDelete) {
    return isset($entry['rule']['id']) && (string)$entry['rule']['id'] !== $idToDelete;
}));

// Verificar si se eliminó algo
// Check if something was deleted
if (count($rulesJson['bpfilter']) === $originalCount) {
