function cargarPoliciesNftablesNft_output() {
const chain = "output";
...
.then(text => {
  ...
  mostrarTablaNftablesNft_output();
})
.catch(error => {
  const container = document.getElementById("nftablesrules-output");
...
function mostrarTablaNftablesNft_output() {
  const container = document.getElementById("nftablesrules-output");
  ...
  btnAdd.onclick = () => addNewNftablesNft_output();
  ...
  data.forEach((rule, index) => {
    ...
    btnEditar.onclick = () => editarNftablesNft_output(index, rule, fila, columnas);

    const btnGuardar = document.createElement("button");
    btnGuardar.textContent = LANG.save;
    btnGuardar.className = "guardar";
    btnGuardar.onclick = () => guardarNftablesNft_output(index, rule, fila, columnas);

    const btnEliminar = document.createElement("button");
    btnEliminar.textContent = LANG.delete;
    btnEliminar.className = "eliminar";
    btnEliminar.onclick = () => eliminarNftablesNft_output(index, rule, fila);
...
function addNewNftablesNft_output() {
  console.log("Añadir nueva regla");

  const formData = new FormData();
  formData.append("chain", "output");
  ...
  .then(result => {
    ...
    if (typeof cargarPoliciesNftablesNft_output === "function") {
      cargarPoliciesNftablesNft_output();
    }
  })
  .catch(error => {
...
function eliminarNftablesNft_output(index, rule, row) {
  console.log(`Eliminar regla en índice ${index}`, rule);

  const formData = new FormData();
  formData.append("chain", rule.chain);   // Cadena: output, output, input
...
function guardarNftablesNft_output(index, rule, row, columnas) {
  ...
  columnas.slice(1).forEach((col, i) => {
    ...
    if (col === "meta.iifname") {
      ...
      match.match.right = limpio;
    } else if (col === "meta.oifname") {
      match.match.left.meta = { key: "oifname" };
      match.match.right = limpio;
