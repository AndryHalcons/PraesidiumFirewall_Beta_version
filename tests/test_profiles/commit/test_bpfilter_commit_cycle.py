#!/usr/bin/env python3
"""
Commit/apply real en laboratorio para bpfilter.
...
def mutate_candidate():
    path = CONFIG_DIR / 'rules_bpfilter_human_viewer.json'
    data = load_json(path)
    data.setdefault('_release_test_marker', 'bpfilter_commit_cycle')
    save_json(path, data)


def verify_commit(payload):
    assert_service_active_or_known('bpfilter.service')


commit_cycle('bpfilter', mutate_candidate, verify_commit)
pass_('bpfilter commit cycle', 'commit/apply real verificado y restaurado')
