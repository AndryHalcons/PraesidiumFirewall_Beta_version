#!/usr/bin/env python3
"""
Commit/apply real en laboratorio para nftables.
...
def mutate_candidate():
    path = CONFIG_DIR / 'rules_nftables_human_viewer.json'
    data = load_json(path)
    # ES/EN: Toggle reversible de descripcion no semantica cuando existe; si no, solo reescribe JSON normalizado.
    data.setdefault('_release_test_marker', 'nftables_commit_cycle')
    save_json(path, data)


def verify_commit(payload):
    assert_command_ok('nft ruleset json', ['sudo', '-n', 'nft', '-j', 'list', 'ruleset'])


commit_cycle('nftables', mutate_candidate, verify_commit)
pass_('nftables commit cycle', 'commit/apply real verificado y restaurado')
