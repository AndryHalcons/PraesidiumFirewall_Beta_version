#!/usr/bin/env python3
"""
Commit/apply real en laboratorio para squid.
...
def mutate_candidate():
    path = CONFIG_DIR / 'squid_config' / 'squid_policies.json'
    data = load_json(path)
    data.setdefault('_release_test_marker', 'squid_commit_cycle')
    save_json(path, data)


def verify_commit(payload):
    assert_command_ok('squid parse', ['squid', '-k', 'parse'], timeout=180)


commit_cycle('squid', mutate_candidate, verify_commit)
pass_('squid commit cycle', 'commit/apply real verificado y restaurado')
