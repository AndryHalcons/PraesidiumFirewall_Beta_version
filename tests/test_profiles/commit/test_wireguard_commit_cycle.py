#!/usr/bin/env python3
"""
Commit/apply real en laboratorio para wireguard.
...
def mutate_candidate():
    path = CONFIG_DIR / 'wireguard.json'
    data = load_json(path)
    data.setdefault('_release_test_marker', 'wireguard_commit_cycle')
    save_json(path, data)


def verify_commit(payload):
    assert_command_ok('wg tool available', ['bash', '-lc', 'command -v wg >/dev/null && wg show || true'])


commit_cycle('wireguard', mutate_candidate, verify_commit)
pass_('wireguard commit cycle', 'commit/apply real verificado y restaurado')
