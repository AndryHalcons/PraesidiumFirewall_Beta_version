#!/usr/bin/env python3
"""
Commit/apply real en laboratorio para dnsmasq/DHCP.
...
def mutate_candidate():
    path = CONFIG_DIR / 'dhcp.json'
    data = load_json(path)
    data.setdefault('_release_test_marker', 'dnsmasq_commit_cycle')
    save_json(path, data)


def verify_commit(payload):
    assert_command_ok('dnsmasq config test', ['dnsmasq', '--test'])


commit_cycle('dnsmasq', mutate_candidate, verify_commit)
pass_('dnsmasq commit cycle', 'commit/apply real verificado y restaurado')
