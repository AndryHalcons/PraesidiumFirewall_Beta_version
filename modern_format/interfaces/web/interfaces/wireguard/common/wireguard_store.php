...
function wireguard_generate_keypair(): ?array {
    // Fase 1: pedir a WireGuard una clave privada nueva sin imprimirla al navegador.
    // Phase 1: ask WireGuard for a new private key without printing it to the browser.
    $private = trim((string)shell_exec('wg genkey 2>/dev/null'));
    if ($private === '') return null;

    // Fase 2: derivar la clave pública alimentando wg pubkey por stdin.
    // Phase 2: derive the public key by feeding wg pubkey through stdin.
...
function wireguard_public_key_from_private(string $private): ?string {
    // Fase única: nunca parseamos la clave a mano; delegamos la derivación a wg pubkey.
    // Single phase: never parse the key manually; delegate derivation to wg pubkey.
    $descriptor = [0 => ['pipe', 'r'], 1 => ['pipe', 'w'], 2 => ['pipe', 'w']];
    $process = proc_open('wg pubkey', $descriptor, $pipes);
    if (!is_resource($process)) return null;
    fwrite($pipes[0], $private);
...
function wireguard_client_endpoint_host(array $server): string {
    // Fase 1: si el admin declaró endpoint público, ese valor manda sobre HTTP_HOST.
    // Phase 1: if the admin declared a public endpoint, it wins over HTTP_HOST.
    $configured = trim((string)($server['public_endpoint'] ?? ''));
    if ($configured !== '') return preg_replace('/:\d+$/', '', $configured);

    // Fase 2: fallback cómodo para laboratorio usando el host desde el que se accede a la web.
    // Phase 2: convenient lab fallback using the host used to access the web UI.
...
function wireguard_build_client_config(string $clientName, array $client, string $serverName, array $server): ?string {
    // Fase 1: comprobar que tenemos secretos y datos mínimos para exportar.
    // Phase 1: check that we have the required secrets and minimum export data.
    // Si falta algo devolvemos null para que el endpoint emita un error controlado.
    // If anything is missing, return null so the endpoint can emit a controlled error.
    ...
    // Fase 2: derivar la pública del servidor; el cliente nunca recibe la privada del servidor.
    // Phase 2: derive the server public key; the client never receives the server private key.
    $serverPublic = wireguard_public_key_from_private($serverPrivate);
    if ($serverPublic === null) return null;

    // Fase 3: resolver endpoint externo que usará el dispositivo del usuario final.
    // Phase 3: resolve the external endpoint used by the final user's device.
    ...
    // Fase 4: construir el bloque [Interface] del cliente.
    // Phase 4: build the client's [Interface] block.
    $lines = [
    ...
    // Fase 5: construir el bloque [Peer] que apunta al firewall Praesidium.
    // Phase 5: build the [Peer] block pointing to the Praesidium firewall.
    $lines[] = '[Peer]';
...
function wireguard_find_client_export(string $clientName, array $config): ?array {
    // Fase 1: localizar el cliente por el nombre que vino en el botón de su propia fila.
    // Phase 1: find the client by the name passed from its own row button.
    if (!isset($config['remote_clients'][$clientName])) return null;
    $client = $config['remote_clients'][$clientName];

    // Fase 2: resolver el servidor VPN asociado; sin servidor no hay endpoint exportable.
    // Phase 2: resolve the associated VPN server; without it there is no exportable endpoint.
...
function wireguard_validate_rule(string $alias, array $rule, ?array $config = null, string $entryName = ''): array {
    // Fase 1: normalizar entrada y resolver qué sección WireGuard se está editando.
    // Phase 1: normalize input and resolve which WireGuard section is being edited.
    $rule = wireguard_trim_rule($rule); $config = $config ?? wireguard_empty_config(); $section = wireguard_section_for_alias($alias) ?? '';

    // Fase 2: validar lista blanca de campos excluyendo columnas botón no persistentes.
    // Phase 2: validate the field allow-list while excluding non-persistent button columns.
    ...
    // Fase 3: validaciones genéricas compartidas por los tres escenarios.
    // Phase 3: generic validations shared by the three scenarios.
    ...
    // Fase 4: delegar las reglas propias de cada escenario para mantener este validador legible.
    // Phase 4: delegate scenario-specific rules to keep this validator readable.
