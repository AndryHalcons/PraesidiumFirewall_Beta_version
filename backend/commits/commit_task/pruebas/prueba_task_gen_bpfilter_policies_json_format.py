...
def saniticed_bpfilter_format(rule):
    ...
    chain = convert_bpfilter.verify_chain(rule.get("hook"),rule.get("chain"))
...
def gen_bpf_txt_formt(allRules):
    from collections import defaultdict

    outputPath = "/home/praesidium/PraesidiumFirewall/backend/commits/commit_task/pruebas/pruebas_machine_format.txt"
    ...
    for chain, rules in chains.items():
        ...
        # Construimos el encabezado con ifindex si está presente
        if iface:
            output_lines.append(f"chain {chain} {hook}{{ifindex={iface}}} {action}")
        else:
            #output_lines.append(f"chain {chain} {hook} {action}")
            output_lines.append(f"chain {chain} {hook}{{}} {action}")


        #output_lines.append("    counters policy 0 packets 0 bytes; error 0 packets 0 bytes")
        ...
        for rule in rules:
            ...
            output_lines.append(f"        {rule.get('action', 'ACCEPT')}")

    try:
        with open(outputPath, "w") as f:
            f.write("\n".join(output_lines))
        print(f"Archivo de formato bpfilter guardado en: {outputPath}")
...
def task_gen_bpfilter_policies_json_format():
    #inputPath = "/var/www/config/rules_bpfilter_human_viewer.json"
    inputPath = "/home/praesidium/PraesidiumFirewall/data/rules_bpfilter_human_viewer.json"
